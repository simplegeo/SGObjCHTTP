...
#endif
#import "SGASIInputStream.h"
#import "SGASIDataDecompressor.h"
#import "SGASIDataCompressor.h"

// Automatically set on build
NSString *SGASIHTTPRequestVersion = @"v1.8.1-8 2011-06-05";

static NSString *defaultUserAgent = nil;

NSString* const NetworkRequestErrorDomain = @"SGASIHTTPRequestErrorDomain";

static NSString *SGASIHTTPRequestRunLoopMode = @"SGASIHTTPRequestRunLoopMode";
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((SGASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *SGASIRequestCancelledError;
static NSError *SGASIRequestTimedOutError;
static NSError *SGASIAuthenticationError;
static NSError *SGASIUnableToCreateRequestError;
static NSError *SGASITooMuchRedirectionError;
...
unsigned long const SGASIWWANBandwidthThrottleAmount = 14800;
...
// When using SGASIAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <SGASICacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [SGASIHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
+ (void)performInvocation:(NSInvocation *)invocation onTarget:(id *)target releSGASIngObject:(id)objectToRelease;
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(SGASIHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(SGASIBSGASIcBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(SGASIBSGASIcBlock)block;
#endif
...
@property (assign) SGASIAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) SGASIHTTPRequest *PACFileRequest;
...
@implementation SGASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [SGASIHTTPRequest class]) {
		...
		SGASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:SGASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
		SGASIAuthenticationError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:SGASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		SGASIRequestCancelledError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:SGASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		SGASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:SGASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		SGASITooMuchRedirectionError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:SGASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:SGASIUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <SGASICacheDelegate>)cache andCachePolicy:(SGASICachePolicy)policy
{
	SGASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:SGASINoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![SGASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [SGASIDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	[self failWithError:SGASIRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [SGASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:SGASIHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		if ([SGASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an SGASINetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:SGASIUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an SGASINetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (SGASIS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (SGASIAskServerIfModifiedWhenStaleCachePolicy|SGASIAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)applyAuthorizationHeader
{
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		// If we have bSGASIc authentication explicitly set and a username and password set on the request, add a bSGASIc auth header
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			[self addBSGASIcAuthenticationHeaderWithUsername:[self username] andPassword:[self password]];
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
					...
					// If the Authentication key is not set, these credentials were stored after a username and password set on a previous request passed bSGASIc authentication
					// When this happens, we'll need to create the Authorization header ourselves
						...
						[self addBSGASIcAuthenticationHeaderWithUsername:[usernameAndPassword objectForKey:(NSString *)kCFHTTPAuthenticationUsername] andPassword:[usernameAndPassword objectForKey:(NSString *)kCFHTTPAuthenticationPassword]];
						#if DEBUG_HTTP_AUTHENTICATION
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [SGASIHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[SGASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[SGASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[SGASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[SGASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[(NSInputStream *)CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream]) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[SGASIHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("SGASIStreamID"), [[self connectionInfo] objectForKey:@"id"]);
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[SGASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[SGASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)performRedirect
{
	...
	if ([self redirectCount] > RedirectionLimit) {
		...
		[self failWithError:SGASITooMuchRedirectionError];
		[self setComplete:YES];
	} else {
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:SGASIRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[SGASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
{
	SGASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[SGASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[SGASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[SGASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[SGASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[SGASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[SGASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[SGASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[SGASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[SGASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[SGASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[SGASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[SGASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[SGASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[SGASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[SGASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        SEL callback = @selector(performInvocation:onTarget:releSGASIngObject:);
        NSMethodSignature *cbSignature = [SGASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[SGASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	NSLog(@"[STATUS] Request %@: %@",self,(theError == SGASIRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != SGASIAuthenticationErrorType && [theError code] != SGASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
	...
	// If we have cached data, use it and ignore the error when using SGASIFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & SGASIFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	SGASIHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an SGASINetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:SGASIHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:SGASIProxyAuthenticationNeeded];
	} else {
		#if DEBUG_HTTP_AUTHENTICATION
	...
	if (![self authenticationNeeded]) {
		...
		// Did we get here without an authentication challenge? (which can happen when shouldPresentCredentialsBeforeChallenge is YES and bSGASIc auth was successful)
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[SGASIHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		SGASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (BOOL)willRedirect
{
	...
	// Note that SGASIHTTPRequest does not currently support 305 Use Proxy
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[SGASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[SGASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	SGASIHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an SGASINetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [SGASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [SGASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		NSLog(@"[AUTH] Request %@ received credentials from its delegate or an SGASIAuthenticationDialog, will retry",self);
	#endif
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		NSLog(@"[AUTH] Request %@ had authentication cancelled by its delegate or an SGASIAuthenticationDialog",self);
	#endif
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == SGASIProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[SGASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[SGASIDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[SGASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[SGASIDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
	...
	// If request has asked delegate or SGASIAuthenticationDialog for credentials
...
- (void)markAsFinished
{
    ...
    BOOL wSGASInProgress = inProgress;
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-SGASIHTTPRequest-Response-Status-Code"] intValue]];
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (SGASIHTTPRequest does not support file:// urls)
	...
	// Create an SGASIHTTPRequest to fetch the PAC file
	SGASIHTTPRequest *PACRequest = [SGASIHTTPRequest requestWithURL:[self PACurl]];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	SGASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[SGASIHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[SGASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[SGASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[SGASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[SGASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:SGASICacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and SGASIHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
				...
				// We won't bother to check for systems prior to 10.4, since SGASIHTTPRequest only works on 10.5+
				OSErr err;
...
- (void)performThrottling
{
	...
	[SGASIHTTPRequest measureBandwidthUsage];
	if ([SGASIHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[SGASIHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an SGASINetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[SGASIHTTPRequest throttleBandwidthForWWANUsingLimit:SGASIWWANBandwidthThrottleAmount];
	} else {
		[SGASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[SGASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[SGASIHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[SGASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[SGReachability reachabilityForInternetConnection] startNotifier];
...
{
	return ([[SGReachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);	
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [SGASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (NSString*)base64forData:(NSData*)theData {
    ...
    static char table[] = "ABCDEFGHISGJKLMNOPQRSTUVWXYZabcdefghiSGJKlmnopqrstuvwxyz0123456789+/=";
