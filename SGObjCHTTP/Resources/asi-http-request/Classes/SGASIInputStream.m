...
@implementation SGASIInputStream

+ (void)initialize
{
	if (self == [SGASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(SGASIHTTPRequest *)theRequest
{
	SGASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(SGASIHTTPRequest *)theRequest
{
	SGASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask SGASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([SGASIHTTPRequest isBandwidthThrottled]) {
		toRead = [SGASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	[SGASIHTTPRequest incrementBandwidthUsedInLastSecond:toRead];
	[readLock unlock];
