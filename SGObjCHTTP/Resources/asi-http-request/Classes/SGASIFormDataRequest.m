...
// Private stuff
@interface SGASIFormDataRequest ()
...
@implementation SGASIFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:SGASIURLEncodedPostFormat];
...
- (void)addFile:(NSString *)filePath withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if (!fileExists || isDirectory) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:SGASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",filePath],NSLocalizedDescriptionKey,nil]]];
	}
	...
	if (!contentType) {
		contentType = [SGASIHTTPRequest mimeTypeForFileAtPath:filePath];
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == SGASIURLEncodedPostFormat) {
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:SGASIMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	SGASIFormDataRequest *newRequest = [super copyWithZone:zone];
