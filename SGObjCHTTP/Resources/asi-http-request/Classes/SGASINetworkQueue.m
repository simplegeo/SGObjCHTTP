...
- (void)dealloc
{
	...
	for (SGASIHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[SGASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[SGASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
{
	if ([operation isKindOfClass:[SGASIHTTPRequest class]]) {
		
		SGASIHTTPRequest *request = (SGASIHTTPRequest *)operation;
...
// Only add SGASIHTTPRequests to this queue!!
- (void)addOperation:(NSOperation *)operation
{
	if (![operation isKindOfClass:[SGASIHTTPRequest class]]) {
		[NSException raise:@"AttemptToAddInvalidRequest" format:@"Attempted to add an object that was not an SGASIHTTPRequest to an SGASINetworkQueue"];
	}
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				SGASIHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)request:(SGASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	...
	if ([self downloadProgressDelegate]) {
		[SGASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}
...
- (void)request:(SGASIHTTPRequest *)request didSendBytes:(long long)bytes
{
	...
	if ([self uploadProgressDelegate]) {
		[SGASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}
...
{
	SGASINetworkQueue *newQueue = [[[self class] alloc] init];
